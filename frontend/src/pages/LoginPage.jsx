...
function LoginPage({ onNavigate }) {
  ...
  const handleSignIn = async (e) => {
    e.preventDefault()
    try {
      const response = await fetch(`${(import.meta.env.VITE_API_URL || 'https://vyaparsetu-f6yi.onrender.com')}/api/auth/login`, {
        method: 'POST',
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      const response = await fetch(`${(import.meta.env.VITE_API_URL || 'https://vyaparsetu-f6yi.onrender.com')}/api/auth/forgot-password`, {
        method: 'POST',
        headers: {
